...
const LAST_MODEL_KEY = "seosh_last_model_id";
...
export function AIModelSelector({
  ...
  const { data: allModels, isLoading } = trpc.ai.listModels.useQuery();
  const [isOpen, setIsOpen] = useState(false);
  const [selected, setSelected] = useState<Model | null>(null);
  const buttonRef = useRef<HTMLButtonElement>(null);
  const dropdownRef = useRef<HTMLDivElement>(null);

  // Stable ref for onModelSelect to avoid infinite loops
  const onModelSelectRef = useRef(onModelSelect);
  onModelSelectRef.current = onModelSelect;

  // Filter models by user's enabled list from localStorage
  const models = useMemo(() => {
    if (!allModels) return null;
    try {
      const saved = localStorage.getItem("seosh_enabled_models");
      if (saved) {
        const enabled = new Set<string>(JSON.parse(saved));
        const filtered = allModels.filter(m => enabled.has(m.id));
        return filtered.length > 0 ? filtered : allModels; // fallback to all if none enabled
      }
    } catch {}
    return allModels;
  }, [allModels]);
  ...
  useEffect(() => {
    if (models && models.length > 0) {
      // Priority: prop > localStorage > first model
      if (selectedModelId) {
      ...
      try {
        ...
        if (saved) {
          ...
          if (found) {
            setSelected(found);
            onModelSelectRef.current(found.id);
            return;
          }
        }
      } catch {}
      ...
      onModelSelectRef.current(models[0].id);
    }
  ...
  const handleSelect = useCallback((model: Model) => {
    ...
    try { localStorage.setItem(LAST_MODEL_KEY, model.id); } catch {}
  }, [onModelSelect]);
  ...
  const renderDropdown = () => {
    ...
    const isUpwards = spaceBelow < 400 && rect.top > 400;
    ...
    const dropdown = (
      <div 
        ref={dropdownRef}
        className="fixed w-[300px] z-[99999] bg-surface-800 shadow-2xl shadow-black/80 border border-surface-600 rounded-xl overflow-hidden animate-fade-in"
      ...
      >
        ...
        <div className="p-1 max-h-[400px] overflow-y-auto">
          {providerOrder.filter(p => grouped[p]).map(provider => (
            <div key={provider}>
              <div className="px-3 py-1.5 text-[10px] uppercase tracking-wider font-semibold text-surface-500 mt-1 first:mt-0">
                {provider}
              </div>
