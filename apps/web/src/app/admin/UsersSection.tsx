      ...
      <div className="space-y-4">
        <div className="glass-card p-5 border-brand-500/30">
          ...
          <form onSubmit={handleCreateUser} className="space-y-3">
            <input type="text" required value={newUserName} onChange={(e) => setNewUserName(e.target.value)} placeholder="Full Name"
            ...
            <input type="email" required value={newUserEmail} onChange={(e) => setNewUserEmail(e.target.value)} placeholder="Email Address"
              className="w-full px-3 py-2 text-sm rounded-lg bg-surface-900/50 border border-surface-700 text-surface-100 placeholder:text-surface-500 focus:outline-none focus:border-brand-500/50" />
            <input type="text" required value={newUserPassword} onChange={(e) => setNewUserPassword(e.target.value)} placeholder="Password"
              className="w-full px-3 py-2 text-sm rounded-lg bg-surface-900/50 border border-surface-700 text-surface-100 placeholder:text-surface-500 focus:outline-none focus:border-brand-500/50" />
            ...
            {createUserMsg && (
              <p className={`text-xs mt-2 ${createUserMsg.includes("Error") ? "text-red-400" : "text-emerald-400"}`}>{createUserMsg}</p>
            )}
          </form>
        </div>
                ...
                {historyQuery.data?.map((tx) => (
                  <div key={tx.id} className="flex items-center justify-between py-2 px-2 rounded hover:bg-surface-800/30 text-xs">
                    <div>
                      ...
                      <p className="text-surface-600 text-[10px]">{new Date(tx.createdAt).toLocaleString()}</p>
                    </div>
