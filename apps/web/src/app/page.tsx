...
const QUICK_ACTIONS = [
  { key: "autopilot", icon: Zap, href: "/autopilot", gradient: "from-orange-500 to-rose-500", shadow: "shadow-orange-500/20" },
  { key: "semanticCore", icon: Brain, href: "/semantic-core", gradient: "from-cyan-500 to-blue-500", shadow: "shadow-cyan-500/20" },
  { key: "createContent", icon: FileText, href: "/content/new", gradient: "from-emerald-500 to-teal-500", shadow: "shadow-emerald-500/20" },
  { key: "newProject", icon: Plus, href: "/projects/new", gradient: "from-brand-500 to-accent-500", shadow: "shadow-brand-500/20" },
] as const;
...
export default function DashboardPage() {
  ...
  const statsProps = [
    { labelKey: "projects", value: data?.stats.projects ?? 0, change: null, icon: Target },
    { labelKey: "keywords", value: data?.stats.semanticCores ?? 0, change: null, icon: Brain },
    { labelKey: "content",  value: data?.stats.contentPieces ?? 0, change: null, icon: FileText },
    { labelKey: "published", value: data?.stats.published ?? 0, change: null, icon: Rocket },
  ] as const;

  const steps = [
    { textKey: "step1", done: (data?.stats.projects ?? 0) > 0 },
    { textKey: "step2", done: (data?.stats.semanticCores ?? 0) > 0 },
    { textKey: "step3", done: (data?.stats.contentPieces ?? 0) > 0 },
  ] as const;
    ...
    <DashboardLayout>
      <div className="max-w-7xl mx-auto space-y-8 animate-fade-in">
        ...
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-3xl font-bold text-surface-50">
              {t("welcomeTo")} <span className="gradient-text-brand">SEOSH.AI</span>
            </h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {statsProps.map((stat) => (
            <div key={stat.labelKey} className="glass-card glass-card-hover p-5 transition-all duration-300">
              <div className="flex items-center justify-between mb-3">
                <span className="text-sm text-surface-400">{t(`stats.${stat.labelKey}`)}</span>
        ...
        {/* Quick Actions */}
        <div>
          <h2 className="text-lg font-semibold text-surface-100 mb-4">{t("quickActions.title")}</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {QUICK_ACTIONS.map((action) => (
              ...
              >
                ...
                <h3 className="font-semibold text-surface-100 mb-1">{t(`quickActions.${action.key}.title`)}</h3>
                <p className="text-sm text-surface-400 mb-3">{t(`quickActions.${action.key}.description`)}</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 glass-card p-6">
            <h2 className="text-lg font-semibold text-surface-100 mb-4">{t("recentActivity.title")}</h2>
            {recentActivity.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-12 text-center">
                ...
                <p className="text-surface-400 mb-1">{t("recentActivity.emptyTitle")}</p>
          ...
          <div className="glass-card p-6">
            <h2 className="text-lg font-semibold text-surface-100 mb-4">{t("gettingStarted.title")}</h2>
            <div className="space-y-3">
              {steps.map((item, i) => (
                <div
                  key={i}
            ...
            <div className="mt-4 pt-3 border-t border-surface-800/50">
              <div className="flex items-center justify-between text-sm mb-2">
                <span className="text-surface-400">{t("gettingStarted.progress")}</span>
